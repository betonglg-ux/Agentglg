...
- после повторной сверки с текущим `/workspace/AGENTS.md` выявлено и подготовлено к синхронизации реальное расхождение в зеркальных инструкциях `AGENTS.md` и `agent-development/current-agent-instructions.md`;
- корневой `github-mirror-manifest.md` в зеркале признан устаревшим относительно локального файла из `/workspace/memory/` и подготовлен к обновлению без подмены рабочей памяти.

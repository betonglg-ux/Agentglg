...
### Изменено
- исправлены пути и значения по умолчанию в автоматизации GitHub-зеркала: экспорт теперь смотрит на фактический корень `/workspace/memory`, использует ветку `master` и вызывает реальные скрипты из `memory/automation/`;

...
if [[ -f "${PRIMARY_SCRIPT}" ]]; then
  exec python3 "${PRIMARY_SCRIPT}" --workspace /workspace --only-if-changed
fi

if [[ -f "${FALLBACK_SCRIPT}" ]]; then
  exec python3 "${FALLBACK_SCRIPT}" --workspace /workspace --only-if-changed
fi

if [[ -f "${MEMORY_SCRIPT}" ]]; then
  exec python3 "${MEMORY_SCRIPT}" --workspace /workspace --only-if-changed
fi

...
- Если протокол занимает несколько страниц, но в нижнем колонтитуле нумерация дана как часть общего блока документа, например `Страница 1 из 10`, `Страница 2 из 10`, и пользователь отдельно подтвердил, что это не нужно считать ошибкой для такого случая, не выноси это как ошибку по протоколу. Допустимо только один раз кратко попросить пользователя обратить внимание на этот момент без повторения по каждому протоколу. Сверка: колонтитул, правило.

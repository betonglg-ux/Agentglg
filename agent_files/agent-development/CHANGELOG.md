...
### Изменено
- повторно подтверждено правило синхронизации: локальная память в `/workspace/memory` является первичным источником истины, а GitHub-зеркало обновляется только в сторону `текущий агент -> зеркало`;
- служебная копия `agent_files/agent-development/current-agent-instructions.md` выровнена по фактическому `/workspace/AGENTS.md`;
- служебная карточка `agent_files/agent-development/agent-summary.md` пересобрана по текущей структуре шаблонов и рабочей среды;
- `agent_files/agent-development/skills/README.md` приведен к актуальной переносимой версии.

### Примечания
- перед синхронизацией сравнение велось от локальной памяти и локальных инструкций к зеркалу, без автоматической обратной подмены данных из GitHub;
- по основным файлам памяти (`confirmed-error-patterns.md`, `missed-findings-log.md`, `template-notes.md`, `user-confirmed-corrections.md`) новых расхождений относительно зеркала не обнаружено.

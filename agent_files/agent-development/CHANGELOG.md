...
### Изменено
- переносимая копия инструкций в `agent-development/current-agent-instructions.md` приведена в соответствие с актуальным `AGENTS.md`;
- файлы `agent-development/memory-exports/*.md` заполнены реальным содержимым из локальной памяти вместо стартовых заготовок;
- подтверждено правило синхронизации: локальная память в `/workspace/memory` остается первичным источником, а GitHub-зеркало обновляется только в сторону `текущий агент -> зеркало`.

...
- в `agent_files/agent-development/files-index/attached-files-index.md` восстановлена пропущенная строка про сам файл индекса, чтобы служебный список снова совпадал с фактическим составом зеркала;
...
- проверено, что локальная память в `/workspace/memory/memory/` остаётся первичным источником истины, а удалённая ветка не использовалась как основание для обратной подмены.

### Примечания
- перед синхронизацией вручную сверены актуальные инструкции, локальные memory-файлы и зеркальные экспортные копии;
- изменения в зеркало вносились только после подтверждения, что источник истины находится локально.

## 2026-05-06

### Изменено
- после сверки текущей рабочей среды с зеркалом восстановлены актуальные правила по допустимой записи `более 28 суток` для возраста `28` суток;
- в инструкции агента и в зеркальных memory-экспортах закреплено правило про сверку `Hi` и таблицы 2 для УК-протоколов по схеме Г с приложением Б;
- обновлены переносимые дубли подтвержденных пользовательских правил и заметок по шаблонам;
- зеркальные `AGENTS.md` и `agent-development/current-agent-instructions.md` приведены к текущей рабочей версии;

...
- перед каждой синхронизацией сохраняет защитный снимок ключевых файлов памяти в `memory/snapshots/`;
...
- перед каждой попыткой синхронизации делается защитный снимок ключевой памяти в `memory/snapshots/`;

...
Текущая версия автоматизации по умолчанию работает с веткой `master` репозитория `betonglg-ux/Agentglg` и при подготовке зеркала не должна наследовать случайный локальный `origin` от других служебных репозиториев.
